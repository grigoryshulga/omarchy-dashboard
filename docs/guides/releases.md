...
1. Choose a version. Use a patch for fixes, a minor version for compatible
   features, and a major version for breaking changes. The current candidate is **1.8.2**.
2. Update `manifest.json`, the pending section in `CHANGELOG.md`, and
   `docs/releases/1.8.2.md`. While the PR is a draft, keep the changelog marked
...
1. Set the actual release date in the changelog, remove the draft sentence from
   the release notes, and change its compare link to end at `v1.8.2`.
...
4. Fetch the merged branch and tag that exact commit:
   ...
   git tag -a v1.8.2 origin/main -m "Dashboard 1.8.2"
   git push origin v1.8.2
   gh release create v1.8.2 --verify-tag --title "Dashboard 1.8.2" \
     --notes-file docs/releases/1.8.2.md
   ```

   The manifest version must be `1.8.2`. Never move or overwrite a published tag.
