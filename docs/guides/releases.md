...
1. Choose a version. Use a patch for fixes, a minor version for compatible
   features, and a major version for breaking changes. The current candidate is **1.8.3**.
2. Update `manifest.json`, the pending section in `CHANGELOG.md`, and
   `docs/releases/1.8.3.md`. While the PR is a draft, keep the changelog marked
...
1. Set the actual release date in the changelog, remove the draft sentence from
   the release notes, and change its compare link to end at `v1.8.3`.
...
4. Fetch the merged branch and tag that exact commit:
   ...
   git tag -a v1.8.3 origin/main -m "Dashboard 1.8.3"
   git push origin v1.8.3
   gh release create v1.8.3 --verify-tag --title "Dashboard 1.8.3" \
     --notes-file docs/releases/1.8.3.md
   ```

   The manifest version must be `1.8.3`. Never move or overwrite a published tag.
